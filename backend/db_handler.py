...
class DB:
    ...
    def update_certificate_status(self, user_id: str, cert_new_status: bool, new_due_date):
        try:
            sql = 'UPDATE {self.TABLE_NAME} SET active = ?, due_data = ? WHERE user_id = ?'
            self.cur.execute(sql, (cert_new_status, new_due_date, user_id))
            self.con.commit()
            return True
        except Exception as e:
            print(e)
