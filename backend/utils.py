...
def revoke_certificate(signature: bytes, message:bytes, cert: str):
    # cur.execute(f"SELECT * FROM {table_name} WHERE user_id = '{user_id}'")
    # row = cur.fetchone()
    user = get_certificate_by_user()
    pub_key = get_certificate_pubkey(user[1])
    try:
        if check_signature_belongs_to_certificate_valid(message, pub_key, signature):
            sql = 'UPDATE table_name SET active = ? WHERE user_id = ?'
            cur.execute(sql, (False, user[0]))
        else:
